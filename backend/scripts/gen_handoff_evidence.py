...
GOAL = (
    "Search for member 12345 and open the record. Open a new sub-account for "
    "this member. You cannot decide the account type and initial deposit "
    "amount yourself - call stuck and a human colleague will personally "
    "select the type, enter the amount, and submit the form for you. Once "
    "you resume, the sub-account will already be created: do NOT fill in "
    "the form or submit it yourself. Just observe the current screen, "
    "verify it shows the confirmation with a confirmation number, extract "
    "that confirmation number, and report it. If the confirmation is not "
    "yet visible, re-observe once more before concluding - do not attempt "
    "to create another sub-account."
)
...
async def main() -> None:
    ...
    print(f"STUCK -> intervention {iv.intervention_id} reason={iv.reason!r}")
